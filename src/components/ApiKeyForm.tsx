...
const ApiKeyForm: React.FC = () => {
  ...
  return <div className="p-6 rounded-lg border border-white/10 bg-brain-dark/50 backdrop-blur-md">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-medium text-white">TheBrain API Configuration</h2>
        {isCustomConfig && <Button variant="ghost" size="sm" onClick={handleReset} className="text-white/70 hover:text-white hover:bg-brain-dark/50">
            <RotateCcw className="h-3.5 w-3.5 mr-1" />
            Reset to Default
          </Button>}
      </div>
      
      {validationError && <Alert variant="destructive" className="bg-red-900/30 border-red-800 text-white mb-4">
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          <FormField control={form.control} name="brainId" render={({
          field
        }) => <FormItem>
                <FormLabel className="text-white">Brain ID</FormLabel>
                <FormControl>
                  <Input placeholder="Enter your Brain ID" className="bg-brain-dark/30 border-white/20 text-white" {...field} />
                </FormControl>
              </FormItem>} />
          ...
          field
                ...
                <FormControl>
                  <Input placeholder="Enter your API Key" className="bg-brain-dark/30 border-white/20 text-white" {...field} />
                </FormControl>
              </FormItem>} />
          
          <FormField control={form.control} name="baseUrl" render={({
          field
        }) => <FormItem>
                <FormLabel className="text-white">Base URL</FormLabel>
                <FormControl>
                  <Input placeholder="API Base URL" className="bg-brain-dark/30 border-white/20 text-white" {...field} />
                </FormControl>
                <FormDescription className="text-white/60"></FormDescription>
              </FormItem>} />
          
          <div className="flex gap-2 pt-2">
            <Button type="submit" disabled={isSubmitting} className="bg-brain-secondary hover:bg-brain-secondary/80 text-white">
              {isSubmitting ? <>
            ...
            <Button type="button" variant="outline" onClick={validateApiAfterSave} disabled={isValidating} className="border-white/20 hover:bg-white/10">
              {isValidating ? <>
                  <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                  Validating...
                </> : <>
