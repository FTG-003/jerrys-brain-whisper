...
const JerrysBrainChat: React.FC = () => {
  ...
  const handleThoughtClick = async (thought: ApiThoughtNode) => {
    setIsThinking(true);
    try {
      const [thoughtDetails, relatedThoughts] = await Promise.all([getThought(thought.id), getRelatedThoughts(thought.id)]);
      const botResponseContent = generateThoughtExplorationResponse(thoughtDetails, relatedThoughts);
      const newBotMessage: Message = {
        id: Date.now().toString(),
        content: botResponseContent,
        isUser: false
  ...
  const handleSuggestionClick = async (thoughtId: string) => {
    setIsThinking(true);
    try {
      const [thoughtDetails, relatedThoughts] = await Promise.all([getThought(thoughtId), getRelatedThoughts(thoughtId)]);
      const botResponseContent = generateThoughtExplorationResponse(thoughtDetails, relatedThoughts);
      const newBotMessage: Message = {
        id: Date.now().toString(),
        content: botResponseContent,
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="backdrop-blur-sm bg-white/5 border-b border-white/10 p-2 flex justify-between items-center">
            <div className="flex gap-2">
              ...
              <Button variant="outline" size="sm" onClick={handleZoomOut} className="bg-brain-dark/70 hover:bg-brain-primary border-brain-light/30 text-white">
                <ZoomOut className="h-4 w-4" />
              </Button>
            </div>
            <div className="text-sm text-brain-light">
              {thoughtResults?.mainThought ? `Exploring: ${thoughtResults.mainThought.name}` : 'Search for a concept to begin'}
            </div>
          </div>
          ...
          <div className="flex-1 overflow-hidden bg-gradient-to-b from-brain-dark to-brain-primary/70">
            ...
            transformOrigin: 'center'
                ...
                <ThoughtGraph centralThought={thoughtResults.mainThought} relatedThoughts={thoughtResults.relatedThoughts.slice(0, 12)} onNodeClick={handleThoughtClick} className="w-full h-full" />
              </div> : <div className="flex items-center justify-center h-full flex-col">
          ...
          <div className="backdrop-blur-sm bg-brain-dark/70 border-t border-white/10 p-4">
            <div className="max-w-4xl mx-auto flex gap-2">
              <Input value={input} onChange={e => setInput(e.target.value)} onKeyPress={handleKeyPress} placeholder="Search Jerry's Brain (e.g. trust, capitalism, AI)..." className="flex-1 bg-white/10 border-white/20 text-white placeholder:text-white/50" disabled={isThinking} />
              <Button onClick={handleSend} disabled={!input.trim() || isThinking} className="bg-brain-secondary hover:bg-brain-secondary/80 text-white">
                {isThinking ? <Loader2 className="h-4 w-4 animate-spin" /> : <Search className="h-4 w-4" />}
              </Button>
            </div>
          </div>
        </div>
        ...
        <div className={`border-l border-white/10 backdrop-blur-sm bg-brain-dark/80 overflow-hidden transition-all duration-300 ease-in-out ${showChat ? 'w-1/3 max-w-md' : 'w-0 max-w-0 border-l-0'}`}>
          {showChat && <div className="h-full flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto p-4 scrollbar-none">
                {messages.map(message => <ChatMessage key={message.id} content={message.content} isUser={message.isUser} className={message.isUser ? "bg-brain-primary/70 border border-white/10" : "bg-brain-dark/60 border border-white/5"} />)}
                {isThinking && <ChatMessage content={<ThinkingIndicator />} isUser={false} className="bg-brain-dark/60 border border-white/5" />}
                <div ref={endOfMessagesRef} />
              </div>
              
              {thoughtResults && thoughtResults.relatedThoughts.length > 0 && <div className="p-4 border-t border-white/10 bg-brain-primary/40">
                  ...
                  <div className="max-h-60 overflow-y-auto scrollbar-none">
                    {thoughtResults.relatedThoughts.map(thought => <ThoughtNode key={thought.id} thought={thought} onClick={handleThoughtClick} className="mb-2 hover:bg-white/5" />)}
                  </div>
                </div>}
            </div>}
        </div>
